...
export async function generateMetadata({
  ...
  if (!brand) return {};
  const title = `Alle ${brand.label}-Modelle | EL Fix Mobile Wien`;
  const description = `Vollständige Liste aller ${brand.label}-Modelle, die wir in Wien 1220 Aspern reparieren. Festpreise, 12 Monate Garantie, Express-Service.`;
  return {
    title,
    description,
    alternates: {
      ...
      >
        <div className="mx-auto max-w-5xl px-6 py-20 md:px-8 md:py-28">
          <p className="text-[12px] font-medium uppercase tracking-[0.22em] text-white/55">
            {brand.label.toUpperCase()} REPARATUR
          </p>
          <h1 className="mt-4 text-[clamp(2.25rem,6vw,4.5rem)] font-semibold leading-[1.04] tracking-[-0.03em]">
            Alle {brand.label}-Modelle.
          </h1>
          <p className="mt-6 max-w-2xl text-[18px] leading-[1.55] text-white/75">
            {models.length} Modelle, alle reparierbar in Wien 1220 Aspern.
            Festpreis vor der Reparatur, 12 Monate Garantie, Express-Service in
            30 Minuten möglich.
          </p>
        </div>
      </section>
      ...
      <section className="bg-[var(--color-bg-secondary)] text-[var(--color-text-dark)]">
        <div className="mx-auto max-w-5xl px-6 py-16 md:px-8 md:py-24">
          {groups.map((g) => (
            <div key={String(g.year)} className="mb-12">
              <h2 className="text-[clamp(1.5rem,3vw,2rem)] font-semibold tracking-[-0.02em]">
                {g.year === "older" ? "Ältere Modelle" : g.year}
              </h2>
          ...
          <p className="mt-10 text-[14px] text-[#525257]">
            Modell nicht dabei? Wir reparieren auch ältere {brand.label}-Geräte
            auf Anfrage —{" "}
          ...
          <p className="mt-10 text-[13px] text-[#86868B]">
            ...
            >
              ← Zurück zur {brand.label}-Übersicht
            </Link>
          </p>
        </div>
      </section>
